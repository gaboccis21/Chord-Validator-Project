                ...
                <div className="flex flex-wrap gap-2">
                  {liveSuggestions.map((chord, i) => (
                    <button
                      ...
                      className="px-3 py-1 bg-primary/50 hover:bg-primary text-primary-foreground rounded-full text-xs font-mono transition-colors"
                    >
                      {chord}
                    </button>
                  ))}
              ...
              <div className="grid grid-cols-2 gap-2">
                {getExamples().map((ex, i) => (
                  <button
                    ...
                    className="text-left p-2.5 bg-background/50 hover:bg-primary/10 rounded-lg border border-primary/20 hover:border-primary/50 transition-all"
                  >
              ...
              <div className="grid grid-cols-2 gap-2">
                {getInvalidExamples().map((ex, i) => (
                  <button
                    key={i}
                    onClick={() => handleExample(ex.chords)}
                    className="text-left p-2.5 bg-background/50 hover:bg-destructive/10 rounded-lg border border-destructive/20 hover:border-destructive/50 transition-all"
                  >
                ...
                <div className="flex flex-wrap items-center gap-1">
                  {chordHistory.map((item, i) => (
                    <React.Fragment key={i}>
                      <span className="px-2.5 py-1 bg-primary text-primary-foreground rounded-full text-xs font-mono">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                {result.suggestions && result.suggestions.length > 0 && (
                  <div className="mt-4 bg-background/50 rounded-lg p-3 border border-primary/20">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {result.suggestions.map((chord: string, i: number) => (
                        <button
                          ...
                          className="px-3 py-1 bg-primary hover:bg-primary/90 text-primary-foreground rounded-full text-xs font-mono transition-colors"
                        >
                          {chord}
                        </button>
                      ))}
